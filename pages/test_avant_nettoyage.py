...
selected_df_name = st.sidebar.selectbox("Choisissez un DataFrame :", list(dict_dfs_load.keys()))
...
filtered_df = selected_df[(selected_df['Time'] >= start_time) & (selected_df['Time'] <= end_time)]
...
y_values = [1] * len(filtered_df)  # Les y-values sont toutes égales ici, mais tu peux les ajuster
fig.add_trace(
    go.Scatter(
        x=x_values,
        y=y_values,
        mode="markers",
        marker=dict(
            color=filtered_df["Label"].map(label_color_map),  # Appliquer les couleurs en fonction du label
...
fig.update_layout(
    ...
    xaxis=dict(
        title="Temps",  # Intitulé de l'axe X : "Temps"
        showgrid=True
    ),
    yaxis=dict(
...
fig = px.line(valv_df, x='Time', y=valv_columns, title=f"Visualisation des colonnes Valv - Entre {start_time} et {end_time}")
fig.update_layout(
    height=300,
    width=1400,
    template="simple_white",
