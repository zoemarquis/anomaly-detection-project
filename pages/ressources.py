...
for i, row in time_measures.iterrows():
    fig.add_trace(go.Bar(
        ...
        x=[row['fit_time']],    # Chaque barre a une longueur qui est la valeur du temps d'entraînement
        name=f"Temps d'Entraînement ({row['model_type']})",
        orientation='h',  # Spécifier que les barres sont horizontales
        marker=dict(color=default_colors[colors_model_names.get(row['model_type'], 'blue')]),
...
for i, row in time_measures.iterrows():
    fig.add_trace(go.Bar(
        y=[row['model_type']],  # Chaque modèle est sur l'axe Y
        x=[row['predict_time']],  # Chaque barre a une longueur qui est la valeur du temps de prédiction
