...
def get_df_encoded(df): # ici one hot, d'autres sont possibles (genre le label encoder ?)
...
# 1b 
df_phy_att_encoded = pd.concat([df_phy_1_encoded, df_phy_2_encoded, df_phy_3_encoded, df_phy_4_encoded])
df_phy_norm_encoded = df_phy_norm_encoded

# 2a
df_phy_all_num = pd.concat([df_phy_1_num, df_phy_2_num, df_phy_3_num, df_phy_4_num, df_phy_norm_num])

# 2b
df_phy_all_encoded = pd.concat([df_phy_1_encoded, df_phy_2_encoded, df_phy_3_encoded, df_phy_4_encoded, df_phy_norm_encoded])
...
fig_att = px.imshow(corr_att_num, title="Matrice de Corrélations - Attaque", color_continuous_scale='RdBu', zmin=-1, zmax=1)
fig_norm = px.imshow(corr_norm_num, title="Matrice de Corrélations - Benign", color_continuous_scale='RdBu', zmin=-1, zmax=1)
fig_all = px.imshow(corr_all_num, title="Matrice de Corrélations - Tous les Datasets", color_continuous_scale='RdBu', zmin=-1, zmax=1)
...
fig = px.scatter(pca_df, x='PC1', y='PC2', title='PCA - Projection des Données',
...
# 2. PCA
pca = PCA(n_components=len(numeric_cols))  # Ajuster le nombre de composantes selon le besoin
X_pca = pca.fit_transform(X_scaled)
...
fig_eigen = go.Figure()
fig_eigen.add_trace(go.Scatter(x=[f'PC{i+1}' for i in range(len(explained_variance))], 
                                y=explained_variance,
                                mode='lines+markers', 
                                name='Variance Expliquée',
                                marker=dict(color='blue')))

fig_eigen.update_layout(title='Valeurs Propres des Composantes',
...
fig_heatmap = px.imshow(components_df, title='Heatmap des Composantes Principales',
